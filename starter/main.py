...
def load_saved_objects():
    ...
    try:
        with open(os.path.join(SAVE_PATH, "trained_model.pkl"), "rb") as model_file, open(
            os.path.join(SAVE_PATH, "encoder.pkl"), "rb"
        ) as encoder_file, open(os.path.join(SAVE_PATH, "labelizer.pkl"), "rb") as lb_file:
...
def predict(input: ModelInput):
    ...
    X, _, _, _ = process_data(
        input_df, categorical_features=categorical_features, training=False, encoder=encoder, lb=lb
    )
